...
const highlightedOutput = computed(() => {
  ...
  try {
    return hljs.highlight(output.value, { language: props.language === 'html' ? 'xml' : props.language }).value
  } catch {
    return output.value
  }
})
  ...
  <div class="flex-1 flex flex-col min-h-0">
    ...
    <div class="h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center px-4 gap-2 flex-shrink-0">
      ...
      <div class="ml-auto flex items-center gap-4">
        ...
        <Label v-if="language === 'javascript'" class="flex items-center gap-2 text-sm text-gray-600 font-normal cursor-pointer">
          <Checkbox v-model:checked="semiColons" />
          使用分号
        </Label>

        <Label v-if="language !== 'html'" class="flex items-center gap-2 text-sm text-gray-600 font-normal cursor-pointer">
